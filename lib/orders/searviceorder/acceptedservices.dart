...
import 'accetedservicesdetail.dart';


class AcceptedServices extends StatefulWidget {
  const AcceptedServices({Key? key,}) : super(key: key);


  @override
  State<AcceptedServices> createState() => _AcceptedServicesState();
}

class _AcceptedServicesState extends State<AcceptedServices> {
  @override
  void initState() {
    ...
    return SizedBox(
      ...
      child: SingleChildScrollView(
        ...
        child: Padding(
            ...
            child:StreamBuilder(
              stream:CheckoutApi().getAcceptedServiceOrders(),
              builder: (context, snapshot) {
                if( snapshot.hasData ){
                  var data=jsonDecode(snapshot.data!.body);
                  print("datata usbbffb $data");
                  return snapshot.data?.statusCode==200
                      ?
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      AutoSizeText("Accepted Services (${data["data"].length })",
                                                  ...
                                                  child: InkWell(
                                                    onTap: () {
                                                      Navigator.push(context, PageTransition(
                                                          ...
                                                          childCurrent:  const AcceptedServices(),
                                                          child:  AcceptedServicesDetails(orderDetails:data["data"][index], orderId: data["data"][index]["order_id"].toString(),)));
                                                    },
                                                    child: Row(
                                                      mainAxisAlignment:
                                                      MainAxisAlignment
                                        ...
                                        SizedBox(
                                          child: Padding(
                                            padding: EdgeInsets.all(
                                                size.height * 0.02),
                                            child: AutoSizeText(
                                                "Price -\u{20B9} ${data["data"][index]["total_amount"].toStringAsFixed(2).toString()}  | Delivery- 10 Days",
                                                style: GoogleFonts.openSans(
                                                    fontSize:
                                                    size.height * 0.019,
                                                    fontWeight:
